...
func (p *PostStore) Post(id uuid.UUID) (gonews.Post, error) {
	...
	if err := p.DB.Get(&post, `SELECT * FROM posts WHERE id =$1`, id); err != nil {
...
func (p *PostStore) PostsByThread(threadID uuid.UUID) ([]gonews.Post, error) {
	...
	if err := p.DB.Select(&posts, `SELECT * FROM posts WHERE thread_id = $1`, threadID); err != nil {
...
func (p *PostStore) CreatePost(t *gonews.Post) error {
	...
	if err := p.DB.Get(t, `INSERT INTO posts VALUES($1 ,$2 ,$3, $4,$5) RETURNING *`,
		t.ID,
		t.ThreadID,
		t.Title,
		t.Content,
...
func (p *PostStore) UpdatePost(t *gonews.Post) error {
	if err := p.DB.Get(t, `UPDATE posts SET thread_id = $1, title = $2, content = $3, votes = $4 WHERE id = $5 RETURNING *`,
		t.ThreadID,
		t.Title,
		t.Content,
		t.Votes,
...
func (p *PostStore) DeletePost(id uuid.UUID) error {
	if _, err := p.DB.Exec(`DELETE FROM posts WHERE id = $1`, id); err != nil {
